...
object EsClient extends SearchImplicits with LazyLogging {
  ...
  def getIndexByAlias(alias: String): List[String] =
    client.execute(getAlias(alias)).await.keys.toList

  def getUniqueIndexByAlias(alias: String): Option[String] =
    getIndexByAlias(alias).headOption

  def indexLowercase[T <: AnyRef: Manifest](
      store: String,
      t: T,
    ...
    client
      .execute(indexInto(
        store,
        manifest[T].runtimeClass.getSimpleName.toLowerCase) doc js refresh doRefresh)
      .await
      .id
  }

  def index[T <: Timestamped: Manifest](indexName: String,
                                        t: T,
                                        refresh: Boolean,
  ...
  def simpleIndex[T <: AnyRef: Manifest](indexName: String,
                                         t: T,
                                         refresh: Boolean,
                                         id: String): String = {
    val json = JacksonConverter.serialize(t)
    ...
    client
      .execute(
        indexInto(indexName, manifest[T].runtimeClass.getSimpleName)
          .withId(id) doc json refresh doRefresh
      )
      .await
      .id
  }
  ...
  def load[T: Manifest](indexName: String,
                        uuid: String,
                        esTypeName: String): Option[T] = {
    val res = client.execute(get(uuid) from indexName -> esTypeName).await
    if (res.found) Some(JacksonConverter.deserialize[T](res.sourceAsString))
    else None
  }

  def loadWithVersion[T: Manifest](indexName: String,
                                   uuid: String): Option[(T, Long)] = {
    val res = client
      .execute(
        get(uuid) from indexName -> manifest[T].runtimeClass.getSimpleName)
      .await
    val maybeT =
      if (res.found) Some(JacksonConverter.deserialize[T](res.sourceAsString))
      else None
  ...
  def delete[T: Manifest](indexName: String,
                          uuid: String,
  ...
  def delete[T: Manifest](indexName: String,
    ...
    client
      .execute(
        esdelete4s(uuid) from indexName -> esDocumentName refresh doRefresh)
      .await
      .found
  }

  def bulk(requests: Seq[BulkCompatibleDefinition]): BulkResponse =
    client.execute(esbulk4s(requests: _*)).await
  ...
  def update[T <: Timestamped: Manifest](indexName: String,
    ...
    update[T](indexName,
              t,
              manifest[T].runtimeClass.getSimpleName,
              upsert,
              refresh)
  ...
  def update[T <: Timestamped: Manifest](indexName: String,
    ...
    val js = JacksonConverter.serialize(t)
    val req
      : UpdateDefinition = esupdate4s(t.uuid) in indexName -> manifest[T].runtimeClass.getSimpleName version version doc js
  ...
  def searchAll[T: Manifest](req: SearchDefinition,
                             fieldsDeserialize: (T, Map[String, Any]) => T = {
    ...
    res.hits.hits.map { (hit: SearchHit) =>
      {
        fieldsDeserialize(JacksonConverter.deserialize[T](hit.sourceAsString),
                          hit.fields)
      }
    }
  }
  ...
  def submit[T, U](req: T)(implicit ev1: HttpExecutable[T, U],
                           ev2: Show[T]): Future[U] = {
    debug(req)
    apply().execute(req)
  }
...
//
//  def bulkBalancedFlow(
//      bulkSize: Int = Settings.ElasticSearch.bulkSize,
